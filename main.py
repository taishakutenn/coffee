...
class ChangingCoffeeInformation(QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
    ...
    def create_new_info(self):
        info = (self.nameSortEdit.text(), self.objarkaEdit.text(), self.molotiOrZernaButton.text(), self.descriptionButton.text(), self.priceEdit.text(), self.volumeButton.text())
        conn = sqlite3.connect("coffee.sqlite")
        cur = conn.cursor()
